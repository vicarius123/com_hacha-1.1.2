...
if (empty($list['direction']))
{
	...
	protected function getListQuery()
	{
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('( a.title LIKE ' . $search . '  OR  a.title_en LIKE ' . $search . '  OR  a.text LIKE ' . $search . ' )');
			}
		}
	...
	public function getItems()
	{
		...
		foreach ($items as $item)
		{
			...
			{
				$values = explode(',', $item->category_id);
				...
				foreach ($values as $value)
				{
					...
					{
						$db = JFactory::getDbo();
						$query = "SELECT id,title FROM #__menu_category WHERE state != '-2' HAVING id LIKE '" . $value . "'";
						$db->setQuery($query);
			...
			$item->category_id = !empty($textValue) ? implode(', ', $textValue) : $item->category_id;
